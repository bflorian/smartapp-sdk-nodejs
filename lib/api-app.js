...
module.exports = class SmartApp {
	...
	async handleOAuthCallback(request) {
		...
		let ctx = await this.withContext({
	...
	async _isAuthorized (req) {
		try {
			if (!this._certKeyId) {
				const lastCertificate = this._contextStore.getLatestCertificate(this._appId)
				this._certKeyId = lastCertificate.keyId;
				this._certKey = lastCertificate.cert;
			}
			...
			if (this._certKeyId !== parsed.keyId)  {
				...
				this._contextStore.putCertificate(this._appId, this._certKeyId, this._certKey)
			}
	...
	async _handleCallback(body, responder) {
		...
		switch (messageType) {
			case 'EVENT': {
				...
				const {locationId, installedAppId} = eventData.installedApp;
				...
				for (const event of eventData.events) {
					switch (event.eventType) {
						case 'INSTALLED_APP_LIFECYCLE_EVENT':
							const installedAppLifecycleEvent = event.installedAppLifecycleEvent
